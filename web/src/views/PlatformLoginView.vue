  ...
  <div class="login">
    <aside class="login-brand">
      <div class="brand-top">
        ...
        <span class="brand-name">OrderFlow · Platform</span>
      </div>
      <h1 class="brand-headline">平台运营管理中心</h1>
      <p class="brand-desc">跨租户总览全平台经营，管理商家入驻与启停。</p>
      <ul class="brand-feats">
        <li><el-icon><DataAnalysis /></el-icon> 全平台订单 / GMV 实时汇总</li>
        <li><el-icon><OfficeBuilding /></el-icon> 多租户隔离，一键切换商家视角</li>
        <li><el-icon><Key /></el-icon> 方法级鉴权，角色越权访问零容忍</li>
      </ul>
    ...
    <main class="login-form-wrap">
      <div class="login-card of-fade-up">
        <h2 class="form-title">平台管理员登录</h2>
        <p class="form-sub">查看并治理所有入驻商家</p>
        ...
        <div class="demo-hint">
          ...
          <code>platform-admin</code> / <code>admin123</code>
        </div>
      </div>
    </main>
  </div>
...
.brand-glow {
  ...
  background: radial-gradient(circle, rgba(191, 219, 254, 0.45), transparent 70%); pointer-events: none;
}
...
.brand-desc { font-size: 15px; color: #dbeafe; max-width: 380px; line-height: 1.7; }
.brand-feats { list-style: none; padding: 0; margin: 40px 0 0; display: grid; gap: 16px; }
.brand-feats li {
  display: flex; align-items: center; gap: 12px; font-size: 14px; color: #eff6ff;
  background: rgba(255, 255, 255, 0.08); padding: 14px 16px; border-radius: 12px;
}
.brand-feats .el-icon { color: #bfdbfe; font-size: 18px; }
...
.demo-hint code { background: #f3f4f6; color: #4f46e5; padding: 2px 8px; border-radius: 6px; font-weight: 600; }
